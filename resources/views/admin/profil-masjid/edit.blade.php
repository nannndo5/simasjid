<x-layouts.dashboard title="Edit Data Profil Masjid">
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-4">
            <h3 class="text-xl font-bold text-white">Edit Data Profil Masjid</h3>
        </div>
        
        <div class="p-6">
            <form action="{{ route('admin.profil-masjid.update', $profilMasjid) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label for="sejarah" class="block text-sm font-semibold text-gray-700 mb-2">
                            Sejarah Singkat / Deskripsi <span class="text-red-500">*</span>
                        </label>
                        <textarea name="sejarah" id="sejarah" rows="6" required class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('sejarah') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan sejarah singkat atau deskripsi masjid">{{ old('sejarah', $profilMasjid->sejarah) }}</textarea>
                        @error('sejarah')
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($profilMasjid->gambar)
                            <div class="mb-4">
                                ...
                                <img src="{{ Storage::url($profilMasjid->gambar) }}" alt="Gambar Profil Masjid" class="h-48 w-auto rounded-lg border border-gray-300">
                            </div>
                        @endif
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.profil-masjid.index') }}" class="inline-flex justify-center items-center px-5 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors">
                        Batal
                    </a>
