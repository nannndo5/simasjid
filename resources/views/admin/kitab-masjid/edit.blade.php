<x-layouts.dashboard title="Edit Data Kitab Masjid">
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-4">
            <h3 class="text-xl font-bold text-white">Edit Data Kitab Masjid</h3>
        </div>
        
        <div class="p-6">
            <form action="{{ route('admin.kitab-masjid.update', $kitabMasjid) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="judul" id="judul" value="{{ old('judul', $kitabMasjid->judul) }}" maxlength="255" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('judul') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan judul kitab">
                        @error('judul')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="8" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('deskripsi') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan deskripsi kitab">{{ old('deskripsi', $kitabMasjid->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($kitabMasjid->gambar)
                            <div class="mb-4">
                                ...
                                <img src="{{ Storage::url($kitabMasjid->gambar) }}" alt="{{ $kitabMasjid->judul ?? 'Kitab' }}" class="h-48 w-auto rounded-lg border border-gray-300">
                            </div>
                        @endif
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.kitab-masjid.index') }}" class="inline-flex justify-center items-center px-5 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors">
                        Batal
                    </a>
