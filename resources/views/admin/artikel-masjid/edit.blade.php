<x-layouts.dashboard title="Edit Data Artikel Masjid">
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-4">
            <h3 class="text-xl font-bold text-white">Edit Data Artikel Masjid</h3>
        </div>
        
        <div class="p-6">
            <form action="{{ route('admin.artikel-masjid.update', $artikelMasjid) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $artikelMasjid->kategori) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('kategori') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Contoh: Akidah, Fiqh, Akhlak">
                        @error('kategori')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="penulis" id="penulis" value="{{ old('penulis', $artikelMasjid->penulis) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('penulis') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan nama penulis artikel">
                        @error('penulis')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="8" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('deskripsi') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan deskripsi atau isi artikel">{{ old('deskripsi', $artikelMasjid->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($artikelMasjid->gambar)
                            <div class="mb-4">
                                ...
                                <img src="{{ Storage::url($artikelMasjid->gambar) }}" alt="{{ $artikelMasjid->judul }}" class="h-48 w-auto rounded-lg border border-gray-300">
                            </div>
                        @endif
