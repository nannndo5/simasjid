<x-layouts.dashboard title="Data Mualaf">
    ...
    <!-- Search Bar -->
    <div class="mb-6">
        <form method="GET" action="{{ route('admin.mualaf.index') }}" class="flex gap-3">
            ...
            @if(request('search'))
                <a href="{{ route('admin.mualaf.index') }}" class="px-5 py-2.5 bg-gray-500 hover:bg-gray-600 text-white rounded-lg font-medium transition-colors">
                    Reset
                </a>
            @endif
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($mualafs as $index => $mualaf)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($mualaf->gambar)
                                    <div class="flex-shrink-0 h-16 w-24">
                                        <img src="{{ Storage::url($mualaf->gambar) }}" alt="Mualaf" class="h-16 w-24 rounded-lg object-cover border border-gray-200">
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-500 max-w-xs truncate">{{ Str::limit($mualaf->deskripsi ?? '', 50) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 flex items-center">
                                    ...
                                    {{ $mualaf->created_at->format('d M Y') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end items-center space-x-2">
                                    <a href="{{ route('admin.mualaf.show', $mualaf) }}" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.mualaf.edit', $mualaf) }}" class="inline-flex items-center justify-center w-8 h-8 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.mualaf.destroy', $mualaf) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data Mualaf ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 font-medium">Tidak ada data Mualaf</p>
                                    <p class="text-gray-400 text-sm mt-1">Mulai dengan menambahkan data Mualaf baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
