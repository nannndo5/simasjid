<x-layouts.dashboard title="Tambah Data AL-Quran Masjid">
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
            <h3 class="text-xl font-bold text-white">Tambah Data AL-Quran Masjid Baru</h3>
        </div>
        
        <div class="p-6">
            <form action="{{ route('admin.alquran-masjid.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="judul" id="judul" value="{{ old('judul') }}" maxlength="255" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('judul') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan judul AL-Quran">
                        @error('judul')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="8" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('deskripsi') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan deskripsi alquran">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
