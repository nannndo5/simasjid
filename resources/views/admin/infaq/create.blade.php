<x-layouts.dashboard title="Tambah Data Infaq">
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
            <h3 class="text-xl font-bold text-white">Tambah Data Infaq Baru</h3>
        </div>
        
        <div class="p-6">
            <form action="{{ route('admin.infaq.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="jenis_transaksi" id="jenis_transaksi" required class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('jenis_transaksi') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                            ...
                            <option value="infaq" {{ old('jenis_transaksi') === 'infaq' ? 'selected' : '' }}>Infaq</option>
                    ...
                    <div>
                        ...
                        <select name="kategori" id="kategori" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('kategori') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                            ...
                            <option value="uang masuk" {{ old('kategori') === 'uang masuk' ? 'selected' : '' }}>Uang Masuk</option>
                            <option value="uang keluar" {{ old('kategori') === 'uang keluar' ? 'selected' : '' }}>Uang Keluar</option>
                        </select>
                        @error('kategori')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="4" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition-colors @error('keterangan') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Masukkan keterangan (opsional)">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.infaq.index') }}" class="inline-flex justify-center items-center px-5 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors">
                        Batal
                    </a>
