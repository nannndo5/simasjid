<div x-data x-on:search-updated.window="$wire.updateSearch($event.detail.search)">
    ...
    @if(empty($kitabs))
        <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-navy-900">Tidak ada kitab ditemukan</h3>
                    ...
                    <div class="p-6">
                        @if($kitab['title'])
                            <h3 class="text-xl font-semibold text-navy-900 mb-2">
                                <a href="{{ $kitab['url'] }}" class="hover:text-green-600 transition-colors">
                                    {{ $kitab['title'] }}
                                </a>
                            </h3>
                        @endif
                        @if(isset($kitab['excerpt']) && $kitab['excerpt'])
                            <p class="text-navy-600 mb-4 line-clamp-3">
                                {{ Str::limit($kitab['excerpt'], 120) }}
                            </p>
                        @endif
                        <a href="{{ $kitab['url'] }}" class="inline-flex items-center text-green-600 hover:text-green-700 font-medium">
                            Baca Selengkapnya
